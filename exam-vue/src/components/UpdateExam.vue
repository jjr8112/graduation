    ...
    <el-main>
      ...
      <el-card v-show="curStep === 2">
        ...
        <el-alert style="margin-top: 15px"
                  :title="examInfo.type === 1? '开放的，任何人都可以进行练习！' : '半开放的，知道密码的人员才可以练习！'"
                  type="warning">
        </el-alert>
...
<script>
  export default {
    ...
    created () {
      //一创建就改变头部的面包屑
      this.$emit('giveChildChangeBreakInfo', '更新练习', '更新练习')
    ...
    methods: {
      ...
      createTagsInParent () {
        ...
        this.tagInfo.map(item => {
          //如果tags全部符合
          if (item.name === '更新练习' && item.url === this.$route.path) {
            flag = true
          } else if (item.name === '更新练习' && item.url !== this.$route.path) {
            this.$emit('updateTagInfo', '更新练习', this.$route.path)
            flag = true
          }
        })
        if (!flag) this.$emit('giveChildAddTag', '更新练习', this.$route.path)
      },
      //根据练习的id查询练习的信息和题目的信息
      async getExamInfo () {
            ...
            if (this.updateExamQuestion.length !== 0) {//自由组卷模式
              ...
              this.$http.post(this.API.updateExamInfo, exam).then((resp) => {
                if (resp.data.code === 200) {
                  ...
                  localStorage.setItem('examDuration' + this.examInfo.examId, this.examInfo.examDuration*60)
