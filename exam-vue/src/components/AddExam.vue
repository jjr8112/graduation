  ...
  <el-container>
    <el-header height="220">
      <el-steps :active="curStep" simple>
        ...
        <el-step title="练习权限" icon="el-icon-lock"></el-step>
    ...
    <el-main>
      ...
      <el-card v-show="curStep === 1">
        ...
        <div v-show="makeModel === 2">
          <el-card>
            <h1>题目列表</h1>
            <el-button type="primary" icon="el-icon-plus" size="small" @click="showAddDialog">添加题目</el-button>
      ...
      <el-card v-show="curStep === 2">
        ...
        <el-alert style="margin-top: 15px"
                  :title="examAuthority === 1? '开放的，任何人都可以进行练习！' : '半开放的，知道密码的人员才可以练习！'"
                  type="warning">
        </el-alert>

        <el-input style="margin-top: 15px;width: 20%" v-model="examPassword" v-show="examAuthority === 2"
                  type="password" show-password placeholder="输入练习密码"></el-input>
      </el-card>
      ...
      <el-card v-show="curStep === 3">
        ...
        <el-form :model="examInfo" :rules="examInfoRules" ref="examInfoForm" label-width="100px">
          ...
          <el-form-item label="练习名称" prop="examName">
            <el-input v-model="examInfo.examName"></el-input>
          </el-form-item>

          <el-form-item label="练习描述" prop="examDesc">
          ...
          <el-form-item label="练习时长(分钟)" prop="examDuration">
            <el-input-number v-model="examInfo.examDuration" :min="1"></el-input-number>
          </el-form-item>

          <el-form-item label="练习开始时间" prop="startTime">
            <el-date-picker style="margin-left: 5px"
                            v-model="examInfo.startTime"
                            type="date" placeholder="练习开始时间">
            </el-date-picker>
          </el-form-item>

          <el-form-item label="练习结束时间" prop="endTime">
            <el-date-picker style="margin-left: 5px"
                            v-model="examInfo.endTime"
                            type="date" placeholder="练习结束时间">
            </el-date-picker>
          </el-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        //组卷模式
        makeModel: 1,
        //添加练习题目信息(makeModel = 1的时候)
        addExamQuestion: [],
        //所有题库信息
        allBank: [],
        //添加练习题目信息(makeModel = 2 的时候)
        addExamQuestion2: [],
        //所有题目的对话框
    ...
    created () {
      //一创建就改变头部的面包屑
      this.$emit('giveChildChangeBreakInfo', '添加练习', '添加练习')
    ...
    methods: {
      ...
      createTagsInParent () {
        ...
        this.tagInfo.map(item => {
          //如果tags全部符合
          if (item.name === '添加练习' && item.url === this.$route.path) {
            flag = true
          } else if (item.name === '添加练习' && item.url !== this.$route.path) {
            this.$emit('updateTagInfo', '添加练习', this.$route.path)
            flag = true
          }
        })
        if (!flag) this.$emit('giveChildAddTag', '添加练习', this.$route.path)
      },
      //获取所有的题库信息
      getBankInfo () {
        ...
        this.$refs['examInfoForm'].validate((valid) => {
          if (valid && (this.addExamQuestion.length !== 0 || this.addExamQuestion2.length !== 0)) {
            ...
            if (this.makeModel === 1 && !this.addExamQuestion.some(item => item.bankId === '')) {
              ...
              this.$message.error('请检查练习规则设置是否完整')
            }
          } else {
            this.$message.error('请检查练习规则设置是否完整')
            return false
