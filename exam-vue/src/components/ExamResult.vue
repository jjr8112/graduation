    ...
    methods: {
      ...
      createTagsInParent () {
        ...
        this.tagInfo.map(item => {
          //如果tags全部符合
          if (item.name === '练习结果' && item.url === this.$route.path) {
            flag = true
          } else if (item.name === '练习结果' && item.url !== this.$route.path) {
            this.$emit('updateTagInfo', '练习结果', this.$route.path)
            flag = true
          }
        })
        if (!flag) this.$emit('giveChildAddTag', '练习结果', this.$route.path)
      },
      //查询用户当时练习的信息
      getExamRecord () {
